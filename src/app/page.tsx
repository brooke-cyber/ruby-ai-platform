...
function ProductDemo() {
  ...
  return (
    <div className="rounded-2xl border border-neutral-200 bg-white shadow-[0_20px_70px_rgba(0,0,0,0.07)] overflow-hidden">
      {/* Top bar with phase indicators */}
      <div className="px-5 py-3.5 border-b border-neutral-100 flex items-center justify-between bg-neutral-50/60">
        <div className="flex items-center gap-5">
          {phaseLabels.map((label, i) => (
            <div key={label} className="flex items-center gap-2">
              ...
              <span className={`text-[13px] font-medium transition-colors duration-300 ${phase >= i ? 'text-neutral-700' : 'text-neutral-300'}`}>{label}</span>
              {i < 2 && <div className={`w-6 h-px transition-colors duration-500 ${phase > i ? 'bg-[#be123c]' : 'bg-neutral-200'}`} />}
            </div>
          ))}
        </div>
        <div className="flex items-center gap-2">
          {phase === 1 && (
            <>
              ...
              <span className="text-[13px] text-[#be123c] font-medium">Drafting</span>
            </>
          )}
          {phase === 2 && <span className="text-[13px] text-[#be123c] font-medium">Ready</span>}
        </div>
      </div>
      ...
      <div className="h-[360px] overflow-hidden">
        {/* Phase 0: Intake form */}
        {phase === 0 && (
          <div className="p-5 space-y-2.5 animate-[fadeSlideIn_0.4s_ease-out]">
            <div className="flex items-center justify-between mb-3">
              <p className="text-[13px] uppercase tracking-[0.12em] text-neutral-400 font-medium">Deal Parameters</p>
              <span className="text-[11px] font-medium text-[#be123c] bg-[#be123c]/5 px-2.5 py-0.5 rounded-full">{demo.type} — {demo.badge}</span>
            </div>
            {demo.fields.map((field, i) => (
              <div
                key={field.label}
                className="flex items-center justify-between py-2.5 px-4 rounded-lg bg-neutral-50/80 border border-neutral-100"
                style={{ animation: `fadeSlideIn 0.3s ease-out ${i * 0.1}s both` }}
              >
                <span className="text-[14px] text-neutral-400">{field.label}</span>
                <span className="text-[14px] text-neutral-800 font-medium">{field.value}</span>
              </div>
            ))}
            <div className="pt-2 flex justify-end">
              <div className="inline-flex items-center gap-1.5 text-[13px] text-[#be123c] font-medium">
        ...
        {phase === 1 && (
          <div className="p-5 animate-[fadeSlideIn_0.3s_ease-out]">
            <div className="flex items-center justify-between mb-3">
              <p className="text-[13px] uppercase tracking-[0.12em] text-neutral-400 font-medium">Building Agreement</p>
              <span className="text-[13px] text-neutral-400">{Math.floor(activeArticle)}/8 articles</span>
            </div>
            <div className="space-y-1">
              {demo.articles.map((article, i) => {
                ...
                return (
                  <div
                    key={article.num}
                    className={`rounded-lg px-4 py-2.5 transition-all duration-500 ${
                      visible ? 'bg-white border border-neutral-100' :
                      active ? 'bg-[#be123c]/[0.03] border border-[#be123c]/20' :
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2.5">
                        {visible ? (
                          <svg className="h-3.5 w-3.5 text-[#be123c] shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>
                        ) : active ? (
                          <span className="relative flex h-3.5 w-3.5 items-center justify-center">
                            <span className="animate-ping absolute h-3 w-3 rounded-full bg-[#be123c] opacity-20" />
                            <span className="h-1.5 w-1.5 rounded-full bg-[#be123c]" />
                          </span>
                        ) : null}
                        <span className={`text-[14px] font-semibold ${active ? 'text-[#be123c]' : 'text-neutral-800'}`}>
                          Art. {article.num} — {article.title}
                        </span>
                      </div>
                    </div>
                    {(visible || active) && (
                      <p className={`text-[13px] mt-1 ml-6 ${active ? 'text-[#be123c]/60' : 'text-neutral-400'}`}>
                        {article.snippet}
                      </p>
                    )}
                  </div>
        ...
        {phase === 2 && (
          <div className="p-5 animate-[fadeSlideIn_0.3s_ease-out]">
            <div className="text-center mb-5">
              <div className="inline-flex items-center justify-center h-10 w-10 rounded-full bg-[#be123c]/10 mb-3">
                <svg className="h-5 w-5 text-[#be123c]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
              </div>
              <p className="text-[14px] font-semibold text-neutral-900">Agreement Ready</p>
              <p className="text-[13px] text-neutral-400 mt-0.5">{demo.stats}</p>
            </div>

            <p className="text-[13px] uppercase tracking-[0.12em] text-neutral-400 font-medium mb-2.5">Compliance Verification</p>
            <div className="space-y-1.5">
              {demo.checks.slice(0, checks).map((check) => (
                <div key={check} className="flex items-center gap-2.5 animate-[fadeSlideIn_0.3s_ease-out]">
                  <svg className="h-3.5 w-3.5 text-[#be123c] shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>
                  <span className="text-[13px] text-neutral-600">{check}</span>
                </div>
              ))}
            </div>
            ...
            {checks >= 5 && (
              <div className="mt-4 flex justify-center">
                <div className="inline-flex items-center gap-2 bg-[#be123c] text-white text-[14px] font-medium px-5 py-2 rounded-lg animate-[fadeSlideIn_0.4s_ease-out]">
      ...
      <div className="px-5 py-3 border-t border-neutral-100 bg-neutral-50/50 flex items-center justify-between">
        <span className="text-[10px] text-neutral-400">{demo.footer}</span>
        <div className="flex items-center gap-3">
          ...
          <span className="text-[10px] text-neutral-400">Canadian Compliance</span>
        </div>
      </div>
    </div>
  );
...
export default function Home() {
  return (
    <div className="min-h-screen bg-white">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 sm:gap-8 max-w-2xl mx-auto">
              {[
                ...
                <div key={s.value} className="text-center">
                  ...
                  <p className="text-[14px] text-neutral-400 leading-relaxed">{s.sub}<br />{s.sub2}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* ═══ HOW IT WORKS ═══ */}
      <section className="px-6 py-28 md:py-36 bg-[#fafaf9]">
      ...
      <section className="px-6 py-28 md:py-36">
        <div className="max-w-5xl mx-auto">
          <FadeIn>
            ...
            <p className="text-[16px] text-neutral-500 leading-[1.7] max-w-xl mb-16">
              Most agreements are covered by our pre-certified base contracts. When your deal needs something more, our AI-powered customization layer handles it.
            </p>
          </FadeIn>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <FadeIn>
              <div className="bg-white border border-neutral-200 rounded-xl p-10 h-full hover:border-[#be123c]/40 hover:shadow-md transition-all duration-300">
                <span className="text-[13px] font-medium tracking-[0.2em] text-[#be123c] uppercase">Layer 1</span>
                <h3 className="font-serif text-2xl text-neutral-900 mt-3 mb-4">Pre-Certified Base Contracts</h3>
            ...
            <FadeIn>
              <div className="bg-white border border-neutral-200 rounded-xl p-10 h-full hover:border-[#be123c]/40 hover:shadow-md transition-all duration-300">
                ...
                <h3 className="font-serif text-2xl text-neutral-900 mt-3 mb-4">AI-Powered Customization</h3>
                <ul className="space-y-3">
                  {[
                    'For modifications beyond the standard options',
                    'AI drafts custom clauses through a guided conversation',
            ...
            <FadeIn>
              <div className="space-y-8">
                {[
                  ...
                  { title: 'Every clause tested for enforceability', desc: 'Our AI applies the same enforceability standards that top Canadian courts require. Restrictive covenants, termination clauses, transfer restrictions — all validated before you see them.' },
      ...
      <section className="px-6 py-28 md:py-36 bg-[#fafaf9]">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {[
              { category: 'Hiring & Team', href: '/documents', items: ['Employment Agreements (Standard & Executive)', 'Fixed-Term & Project-Based Contracts', 'Independent Contractor Agreements', 'Non-Compete & Non-Solicitation', 'Confidentiality & IP Protection', 'Offer Letters'], from: 'From $149', tag: '7 agreement types' },
              { category: 'Equity & Governance', href: '/documents', items: ['Co-Founder & Shareholder Agreements', 'Incorporation & Corporate Formation', 'Joint Venture & Investor Agreements', '50/50 Partnership & Deadlock Resolution', 'Partnership Agreements'], from: 'From $249', tag: '9 agreement types' },
              { category: 'Raising Capital', href: '/documents', items: ['SAFE Agreements (Canadian)', 'Convertible Note Agreements', 'Bilateral Loan Agreements', 'Demand Promissory Notes', 'Revolving Credit Facilities'], from: 'From $349', tag: '5 agreement types' },
              { category: 'Software & Services', href: '/documents', items: ['Service Level Agreements (SaaS)', 'Managed Services Agreements', 'Enterprise Licensing Agreements', 'Terms & Conditions', 'Privacy Policies', 'Master Services Agreements'], from: 'From $349', tag: '6 agreement types' },
              { category: 'Platform & Business', href: '/documents', items: ['Website Terms & Conditions', 'Privacy Policies', 'Partnership Agreements', 'Master Services Agreements'], from: 'From $349', tag: '4 agreement types' },
              { category: 'Creator & Influencer', href: '/documents', items: ['Influencer & Brand Partnership Agreements', 'Privacy Policies for Campaigns', 'Confidentiality & IP Protection', 'Contractor Agreements for Creators', 'Exclusivity & Non-Compete'], from: 'From $249', tag: '5 agreement types' },
            ].map((group) => (
              <FadeIn key={group.category}>
                <Link href={group.href} className="block bg-white border border-neutral-200 rounded-xl p-8 h-full hover:border-[#be123c]/40 hover:shadow-md transition-all duration-300 group">
                  ...
                  <ul className="space-y-2.5 mb-6">
                    {group.items.map((item) => (
                      <li key={item} className="text-[15px] text-neutral-500 flex items-center gap-3">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-10">
              {[
                ...
                <Link key={i} href={item.href} className="text-center group">
                  <div className="font-serif text-3xl text-neutral-900 mb-1 group-hover:text-[#be123c] transition-colors">{item.stat}</div>
      ...
      {/* ═══ FINAL CTA ═══ */}
      <section className="px-6 py-28 md:py-36 border-t border-neutral-100">
        <FadeIn>
